    ...
    for file in files:
        ...
        for comment in comments:
            ...
            response = openai.ChatCompletion.create(
              ...
              messages=[
                  {"role": "system", "content": "You are a helpful assistant."},
                  {"role": "user", "content": f"Generate a code suggestion for this comment: {comment.body}. Highlight only the change required with minimal detail. The language is {file_type}. Please give a concise answer. Here is the diff: {file_diff}"},
              ]
            )
            code_suggestion = response['choices'][0]['message']['content']
            pull.create_issue_comment(code_suggestion)
